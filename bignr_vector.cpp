...
public:
	...
	bignr& operator <<= (int x) {
		if (Length == 1 && Number[0] == 0)
			return *this;
		Number.resize(Length + x);
		move(Number.begin(), Number.begin() + Length, Number.begin() + x);
		fill(Number.begin(), Number.begin() + x, 0);
		Length += x;
		return *this;
	}
	bignr& operator >>= (int x) {
		if (x >= Length)
			*this = 0;
			...
			move(Number.begin() + x, Number.end(), Number.begin());
			Number.resize(Length - x);
			Length -= x;
		}
		return *this;
	...
	bignr& operator += (const bignr& nr2) {
		Length = std::max(Length, nr2.Length);
		Number.resize(Length + 1);
		for (int i = 0; i < nr2.Length; ++i) {
			Number[i] += nr2.Number[i];
			if (Number[i] >= GR)
				Number[i] -= GR, ++Number[i + 1];
		}
		for (int i = nr2.Length; i < Length && Number[i] >= GR; ++i)
			Number[i] -= GR, ++Number[i + 1];
		if (Number.back())
			++Length;
		else
	...
	bignr multiply(bignr nr2) {
		...
		bignr ac = a.multiply(c), bd = b.multiply(d);
		return (ac << (2 * h)) + (((a + b).multiply(c + d) - ac - bd) << h) + bd;
	}
	template<typename T, typename = typename std::enable_if<!std::is_base_of<bignr, T>::value>::type>
	bignr& operator %= (const T& nr2) {
		U r = 0;
